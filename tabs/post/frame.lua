...
function aux.handle.INIT_UI()
    ...
    do
        ...
        label:SetText('Show exclude list')
    ...
    do
        local label = gui.label(frame.inventory, gui.font_size.small)
        label:SetPoint('TOPRIGHT', -18, -50)
    ...
    do
        ...
        inventory_listing = item_listing.new(
            ...
            function(row)
                ...
                row.item:SetPoint('RIGHT', -24, 0)

                local checkbox = gui.checkbox(row)
                checkbox:SetScale(gui.is_blizzard() and .8 or .9)
                checkbox:SetPoint('RIGHT', row, 'RIGHT', -4, 0)
                checkbox:SetScript('OnClick', function()
                    if row.item_record then
                        set_item_excluded(row.item_record.key, this:GetChecked())
                        refresh = true
                    end
                end)
                checkbox:SetScript('OnEnter', function()
                    GameTooltip:SetOwner(this, 'ANCHOR_RIGHT')
                    GameTooltip:SetText('Exclude from Scan Bag and normal Post list')
                    GameTooltip:Show()
                end)
                checkbox:SetScript('OnLeave', function()
                    GameTooltip:Hide()
                end)
                row.exclude_checkbox = checkbox
            end,
            function(row, item_record)
                row.exclude_checkbox:SetChecked(is_item_excluded(item_record.key))
            end
        )
    end
    ...
    do
        ...
        btn:SetWidth(90)
        btn:SetText('Scan Bag')
        btn:SetScript('OnClick', scan_bag_items_click)
        scan_bag_button = btn
    end
    do
    ...
    do
        ...
        checkbox:SetScript('OnClick', function()
            if selected_item then
                set_item_excluded(selected_item.key, this:GetChecked())
                refresh = true
            end
        end)
        local label = gui.label(checkbox, gui.font_size.small)
        label:SetPoint('LEFT', checkbox, 'RIGHT', 4, 1)
        label:SetText('Exclude this item')
