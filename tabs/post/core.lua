...
function set_scan_bag_button_state(active)
	bag_scan_active = active and true or false
	if scan_bag_button then
		scan_bag_button:SetText(bag_scan_active and 'Stop Scan' or 'Scan Bag')
	end
end

function clear_item_scan_records(item_key)
	bid_records[item_key], buyout_records[item_key] = nil, nil
end

function ensure_item_scan_records(item_key)
	bid_records[item_key] = bid_records[item_key] or T.acquire()
	buyout_records[item_key] = buyout_records[item_key] or T.acquire()
end

function is_item_excluded(item_key)
	return read_settings(item_key).hidden
end

function set_item_excluded(item_key, excluded)
	local settings = read_settings(item_key)
	settings.hidden = excluded and true or false
	write_settings(settings, item_key)
	if selected_item and selected_item.key == item_key and hide_checkbox then
		hide_checkbox:SetChecked(settings.hidden)
	end
end

function should_show_excluded_items()
	return show_hidden_checkbox and show_hidden_checkbox:GetChecked() or false
end

function get_scanable_inventory_records()
	local records = aux.values(aux.filter(aux.copy(inventory_records), function(record)
		return record.aux_quantity > 0 and not is_item_excluded(record.key)
	end))
	sort(records, function(a, b) return (a.name or '') < (b.name or '') end)
	return records
end

function scan_bag_items_click()
	if bag_scan_active then
		scan.abort(scan_id)
		refresh = true
		return
	end
	...
	local records = get_scanable_inventory_records()
	if getn(records) == 0 then
		status_bar:update_status(1, 1)
		status_bar:set_text('No non-excluded auctionable bag items to scan')
		refresh = true
		return
	end
	...
	local tracked_keys = T.acquire()
	local queued_item_ids = T.acquire()
	local queries = T.acquire()
	local scan_records = T.acquire()
	local current_index = 0
	local current_record
	for _, record in records do
		tracked_keys[record.key] = true
		clear_item_scan_records(record.key)
		if not queued_item_ids[record.item_id] then
			local query = scan_util.item_query(record.item_id)
			if query then
				queued_item_ids[record.item_id] = true
				tinsert(queries, query)
				tinsert(scan_records, record)
			end
		end
	end
	...
	if getn(queries) == 0 then
		status_bar:update_status(1, 1)
		status_bar:set_text('No scan queries could be created')
		refresh = true
		return
	end
	...
	set_scan_bag_button_state(true)

	status_bar:update_status(0, 0)
	status_bar:set_text(format('Scanning %d bag items...', getn(scan_records)))
	...
	scan_id = scan.start{
		...
		on_start_query = function(query_index)
			...
			current_record = scan_records[query_index]
			status_bar:update_status((query_index - 1) / getn(scan_records), 0)
			status_bar:set_text(format('Scanning %s (%d/%d)', current_record.name or 'item', query_index, getn(scan_records)))
		end,
		on_page_loaded = function(page, total_pages)
			if current_record then
				local progress = ((current_index - 1) + (page / max(total_pages, 1))) / getn(scan_records)
				status_bar:update_status(progress, 0)
				status_bar:set_text(format('Scanning %s (%d/%d) Page %d/%d', current_record.name or 'item', current_index, getn(scan_records), page, total_pages))
			end
		end,
		on_auction = function(auction_record)
			if tracked_keys[auction_record.item_key] then
				record_auction(auction_record.item_key, auction_record.aux_quantity, auction_record.unit_blizzard_bid, auction_record.unit_buyout_price, auction_record.duration, auction_record.owner)
			end
		end,
		on_abort = function()
			set_scan_bag_button_state(false)
			status_bar:update_status(1, 1)
			status_bar:set_text('Bag scan aborted')
			refresh = true
		end,
		on_complete = function()
			for _, record in records do
				ensure_item_scan_records(record.key)
			end
			set_scan_bag_button_state(false)
			status_bar:update_status(1, 1)
			status_bar:set_text(format('Bag scan complete (%d items)', getn(records)))
			refresh = true
		end,
	}
end
...
function update_inventory_listing()
	local show_excluded = should_show_excluded_items()
	local records = aux.values(aux.filter(aux.copy(inventory_records), function(record)
		if record.aux_quantity <= 0 then
			return false
		end
		local excluded = is_item_excluded(record.key)
		return show_excluded and excluded or (not show_excluded and not excluded)
	end))
	sort(records, function(a, b) return (a.name or '') < (b.name or '') end)
...
function start_post_selected_item()
	if selected_item then
	...
	for slot in info.inventory() do
		...
		if item_info and item_info.item_key == item_key then
			if info.auctionable(item_info.tooltip, nil, true) and not item_info.lootable then
				local vendor_price = info.get_vendor_price(item_info.item_id, item_info.charges)
