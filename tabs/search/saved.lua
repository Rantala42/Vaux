...
function aux.handle.LOAD()
	...
	aux.realm_data.shopping_lists = aux.realm_data.shopping_lists or {}
...
function get_shopping_list_filter_string(items)
	...
	for _, item_name in ipairs(items or {}) do
...
local function shopping_list_summary(shopping_list)
	local count = getn(shopping_list.items or {})
...
function update_buy_list_items_listing()
	...
	for i, item_name in ipairs(buy_list_editor.items or {}) do
		tinsert(rows, T.map(
...
function update_buy_lists_listing()
	if not shopping_lists_listing then return end

	local rows = T.acquire()
	for i = 1, getn(shopping_lists or {}) do
	...
	shopping_lists_listing:SetData(rows)
end
...
function load_buy_list_editor(index)
	...
	buy_list_editor.items = aux.copy(shopping_list.items or {})
...
function add_buy_list_item(name)
	...
	for _, existing_name in ipairs(buy_list_editor.items or {}) do
...
function delete_buy_list(index)
	if not shopping_lists[index] then return end

	tremove(shopping_lists, index)
	if buy_list_editor.edit_index == index then
		reset_buy_list_editor()
	elseif buy_list_editor.edit_index and buy_list_editor.edit_index > index then
		buy_list_editor.edit_index = buy_list_editor.edit_index - 1
	end

	update_buy_lists_listing()
end

function move_buy_list_up(index)
	if not shopping_lists[index - 1] then return end
	shopping_lists[index], shopping_lists[index - 1] = shopping_lists[index - 1], shopping_lists[index]
	if buy_list_editor.edit_index == index then
		buy_list_editor.edit_index = index - 1
	elseif buy_list_editor.edit_index == index - 1 then
		buy_list_editor.edit_index = index
	end
	update_buy_lists_listing()
end

function move_buy_list_down(index)
	if not shopping_lists[index + 1] then return end
	shopping_lists[index], shopping_lists[index + 1] = shopping_lists[index + 1], shopping_lists[index]
	if buy_list_editor.edit_index == index then
		buy_list_editor.edit_index = index + 1
...
function save_buy_list()
	...
	if name == '' then
		...
		return
	elseif getn(buy_list_editor.items or {}) == 0 then
...
buy_list_handlers = {
	OnClick = function(_, data, _, button)
		...
		local filter_string = data.list.filter_string or get_shopping_list_filter_string(data.list.items)
		if button == 'LeftButton' and IsShiftKeyDown() then
			set_filter(filter_string)
		elseif button == 'RightButton' and IsShiftKeyDown() then
			add_filter(filter_string)
		...
		elseif button == 'RightButton' then
			gui.menu(
				...
				'Move Up', function() move_buy_list_up(data.index) end,
				'Move Down', function() move_buy_list_down(data.index) end,
	...
	OnEnter = function(_, data)
		...
		GameTooltip:AddLine(' ')
		for _, item_name in ipairs(data.list.items or {}) do
			GameTooltip:AddLine(item_name, 255/255, 254/255, 250/255, true)
		end
		GameTooltip:AddLine(' ')
		GameTooltip:AddLine('Left-click to scan the whole list', .8, .8, .8)
		GameTooltip:AddLine('Shift-left to load the list into the search box', .8, .8, .8)
		GameTooltip:AddLine('Right-click for edit options', .8, .8, .8)
		GameTooltip:Show()
	end,
	OnLeave = function()
		GameTooltip:ClearLines()
