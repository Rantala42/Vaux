...
function aux.handle.INIT_UI()
    ...
    frame.saved.builder = gui.panel(frame.saved)
    frame.saved.builder:SetWidth(364.5)
    frame.saved.builder:SetHeight(220)
    frame.saved.builder:SetPoint('TOPRIGHT', 0, 0)

    frame.saved.recent = gui.panel(frame.saved)
    frame.saved.recent:SetWidth(364.5)
    frame.saved.recent:SetPoint('TOPRIGHT', frame.saved.builder, 'BOTTOMRIGHT', 0, -5)
    ...
    do
        local label = gui.label(frame.saved.builder, gui.font_size.medium)
        label:SetPoint('TOPLEFT', 10, -10)
        label:SetText('Buy List Builder')
    end
    do
        local editbox = gui.editbox(frame.saved.builder)
        editbox:SetPoint('TOPLEFT', 10, -34)
        editbox:SetWidth(210)
        editbox:SetHeight(24)
        editbox.enter = function() buy_list_item_input:SetFocus() end
        local label = gui.label(editbox, gui.font_size.small)
        label:SetPoint('BOTTOMLEFT', editbox, 'TOPLEFT', -2, 1)
        label:SetText('List Name')
        buy_list_name_input = editbox
    end
    do
        local btn = gui.button(frame.saved.builder)
        btn:SetPoint('LEFT', buy_list_name_input, 'RIGHT', 5, 0)
        btn:SetWidth(55)
        btn:SetHeight(24)
        btn:SetText('New')
    ...
    do
        ...
        buy_list_item_input = editbox
    end
    do
        local btn = gui.button(frame.saved.builder)
        btn:SetPoint('LEFT', buy_list_item_input, 'RIGHT', 5, 0)
        btn:SetWidth(95)
        btn:SetHeight(24)
        btn:SetText('Add Item')
        btn:SetScript('OnClick', add_buy_list_item)
    end
    do
        local panel = gui.panel(frame.saved.builder)
        panel:SetPoint('TOPLEFT', buy_list_item_input, 'BOTTOMLEFT', 0, -16)
        panel:SetPoint('BOTTOMRIGHT', -10, 10)
        buy_list_items_panel = panel
    end

    favorite_searches_listing = listing.new(frame.saved.favorite)
    favorite_searches_listing:SetColInfo{{name='Tag', width=.12, align='CENTER'}, {name='Favorite Searches / Buy Lists', width=.88}}
